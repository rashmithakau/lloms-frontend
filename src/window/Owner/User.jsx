        ...
        <div className="flex flex-col md:flex-row gap-4 ">
          <div className="flex flex-wrap gap-2 items-center p-2 border border-pink-300 hover:border-pink-500 rounded-md">
            ...
            {roles.map((role) => (
              <label
                key={role}
                className="flex items-center space-x-1 text-pink-600"
              >
                <input
                  type="checkbox"
        ...
        <table className="min-w-full">
          <thead>
            <tr className="border-b">
              {/* Table Headers */}
              {["userId", "userName", "phoneNumber", "status", "roleId"].map(
                (column) => (
                  <th
                    key={column}
                    className={`px-4 py-3 ${
          ...
          <tbody>
            ...
            {filteredUsers.map((user) => (
              <tr key={user.userId} className="border-b hover:bg-gray-50">
                ...
                {/* User Name */}
                <td className="px-4 py-3 font-medium text-left align-middle">
                  {user.userName}
                </td>

                {/* Phone Number */}
                <td className="px-4 py-3 text-gray-600 text-left align-middle">
                  {user.phoneNumber}
                </td>
